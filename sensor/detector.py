...
try:
    ...
    GPIO.setup(cfg.GPIO_PIN_NO, GPIO.IN)  # Read output from PIR motion sensor

    count = 0
    while True:
        # print "[DEBUG] Wait for probing the movement of people..."
        count += 1
        # Get the return value from the motion sensor.
        detect_status=GPIO.input(cfg.GPIO_PIN_NO)
        ...
        elif detect_status==0:
            print ("[DEBUG] No motion Detected! (%d)" % (count))

        else:
            print ("[DEBUG] Ooops. Fix a sensor error (%d)" % (count))

        # wait for specified seconds after finishing 'for' statement
